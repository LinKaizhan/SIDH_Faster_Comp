...
{
    felm_t  *_2lam1p2, *_2x2plam12p2lam12,*lam122sy2pt0x2st1,*st0x4x2px4slam122x4px4y2sx4y4sy4t1st12, *x4;
    f2elm_t finv[2*t_points], one = {0}, g1;
    ...
    // Pairings with P
    fpcopy(((felm_t*)T4_7)[0], t0);
    fpcopy(((felm_t*)T4_7)[1], t1);
    ...
    for (int k = 0; k < OALICE_BITS/2 - 1; k++) {
        _2lam1p2 = (felm_t*)T4_8 + k;
        _2x2plam12p2lam12 = (felm_t*)T4_9 + k;
        lam122sy2pt0x2st1 = (felm_t*)T4_10 + k;
        st0x4x2px4slam122x4px4y2sx4y4sy4t1st12 = (felm_t*)T4_11 + k;
        x4 = (felm_t*)T4_12 + k;

        for (int j = 0; j < t_points; j++) {
            fpsub(Qj[j]->X[0], *x4, h[0]);
            fpcopy(Qj[j]->X[1], h[1]);
            fp2_conj(h, h);
            fpmul_mont(*_2lam1p2, Qj[j]->X[0], g1[1]);
            fpmul_mont(*_2lam1p2, Qj[j]->X[1], g1[0]);
            fpneg(g1[0]);
            fp2sub(g1, Qj[j]->Y, g1);
            fpadd(g1[1], *lam122sy2pt0x2st1, g1[1]);
            fpadd(Qj[j]->X[0], *_2x2plam12p2lam12, g[0]);
            fpcopy(Qj[j]->X[1], g[1]);
            fp2mul_mont(g, g1, g);
            fpadd(g[1], *st0x4x2px4slam122x4px4y2sx4y4sy4t1st12, g[1]);
            fp2mul_mont(g, h, g);
            fp2sqr_mont(f[j], f[j]);
            fp2sqr_mont(f[j], f[j]);
            fp2mul_mont(f[j], g, f[j]);
...
#if (OALICE_BITS % 2 == 0)
    for (int j = 0; j < t_points; j++) {
        fpcopy(((felm_t*)T4_12)[OALICE_BITS / 2 - 2], t0);
    ...
    for (int j = 0; j < t_points; j++) {
        fpcopy(Qj[j]->X[1], h[1]);
        fpcopy(((felm_t*)T4_13)[2], t0);
        fpsub(Qj[j]->X[0], t0, h[0]);
        fpcopy(((felm_t*)T4_13)[3], t1);
    ...
    for (int k = 0; k < OALICE_BITS / 2 - 1; k++) {
        _2lam1p2 = (felm_t*)T4_2 + k;
        _2x2plam12p2lam12 = (felm_t*)T4_3 + k;
        lam122sy2pt0x2st1 = (felm_t*)T4_4 + k;
        st0x4x2px4slam122x4px4y2sx4y4sy4t1st12 = (felm_t*)T4_5 + k;
        x4 = (felm_t*)T4_6 + k;
        ...
        for (int j = 0; j < t_points; j++) {
            ...
            fpcopy(Qj[j]->X[1], h[1]);
            fp2_conj(h, h);
            fpmul_mont(*_2lam1p2, Qj[j]->X[0], g1[0]);
            fpmul_mont(*_2lam1p2, Qj[j]->X[1], g1[1]);
            fp2sub(g1, Qj[j]->Y, g1);
            fpadd(g1[0], *lam122sy2pt0x2st1, g1[0]);
            fpadd(Qj[j]->X[0], *_2x2plam12p2lam12, g[0]);
            fpcopy(Qj[j]->X[1], g[1]);
            fp2mul_mont(g, g1, g);
            fpadd(g[0], *st0x4x2px4slam122x4px4y2sx4y4sy4t1st12, g[0]);
            fp2mul_mont(g, h, g);
            fp2sqr_mont(f[j + t_points], f[j + t_points]);
            fp2sqr_mont(f[j + t_points], f[j + t_points]);
            fp2mul_mont(f[j + t_points], g, f[j + t_points]);
...
#if (OALICE_BITS % 2 == 0)
    for (int j = 0; j < t_points; j++) {
        fpcopy(((felm_t*)T4_6)[OALICE_BITS / 2 - 2], t0);
        fpsub(Qj[j]->X[0], t0, g[0]);
        fpcopy(Qj[j]->X[1], g[1]);
        fp2sqr_mont(f[j + t_points], f[j + t_points]);
        fp2mul_mont(f[j + t_points], g, f[j + t_points]);
    ...
    for (int j = 0; j < t_points; j++) {
        fpcopy(Qj[j]->X[1], h[1]);
        fpcopy(((felm_t*)T4_13)[0], t0);
        fpsub(Qj[j]->X[0], t0, h[0]);
        fpcopy(((felm_t*)T4_13)[1], t1);
